...
ISSUE_FIXES: Dict[str, Dict] = {
    ...
    "mtk_port_present": {
        ...
        "fix": [
            "Prefer preloader mode: power OFF and plug USB with NO buttons - mtkclient then crashes it into BROM. Do NOT hold Vol Up + Vol Down (that boots Recovery).",
...
CONNECTION_SCENARIOS: List[Dict] = [
    ...
    },
    {
        ...
        "steps": [
            ...
            "Start the tool FIRST (Auto-Connect, or the BROM operation) so it is waiting for the port.",
            "Plug the USB cable into a USB 2.0 (black) port with NO buttons pressed - this is Preloader mode (VID 0E8D, PID 2000).",
            "mtkclient catches the preloader and crashes it into BROM automatically ('crash succeed' -> 'MediaTek USB Port', PID 0003).",
            "NOTE: do NOT hold Vol Up + Vol Down - on the Camon 50 Pro that boots RECOVERY, not BROM.",
            "If the port still doesn't appear, try holding Volume Down only while plugging; last resort is the MT6789 ISP test point.",
        ],
    ...
    },
    {
        ...
        "steps": [
            ...
            "Start the tool FIRST (⚡ MTK BROM tab -> 'Detect BROM Port & Handshake', or Auto-Connect) so it is waiting for the port.",
            "Plug the USB cable into a USB 2.0 (black) port with NO buttons pressed - this is Preloader mode (VID 0E8D, PID 2000).",
            "mtkclient catches the preloader and crashes it into BROM automatically ('crash succeed' -> 'MediaTek USB Port', PID 0003).",
            "IMPORTANT: do NOT hold Vol Up + Vol Down - on the Camon 50 Pro that boots RECOVERY, not BROM. If nothing appears, try Volume Down only, or the MT6789 ISP test point.",
        ...
        "verify": "A COM/tty port with VID 0E8D appears and the handshake returns 0x5F 0xF5 0xAF 0xFA.",
        "failures": [
            {"symptom": "No MTK port appears", "cause": "Missing VCOM driver / charge-only cable / tool started after plug-in", "fix": "Install the MTK VCOM driver, use a data cable, start the tool BEFORE plugging in, and plug with NO buttons (preloader)."},
            {"symptom": "Vol Up + Vol Down boots RECOVERY instead of BROM", "cause": "That combo is Recovery on this phone; stock recovery will not clear the lock (it just reboots)", "fix": "Use preloader mode: power OFF, plug USB with no buttons, and let the tool crash it into BROM. Test point (MT6789 ISP) as last resort."},
            {"symptom": "Port appears for 2s then vanishes", "cause": "Watchdog reset (normal)", "fix": "Start the tool first, then plug in; do not press buttons - the tool re-negotiates and crashes the preloader."},
